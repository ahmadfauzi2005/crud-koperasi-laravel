        ...
        <div class="container">
          ...
          <div class="navbar-collapse collapse justify-content-center" id="navbarDefault">
            <ul class="navbar-nav">
              ...
              <li class="nav-item">
                <a class="nav-link active" href="angsuran">angsuran</a>
              </li>
...
@endif
                            <!-- Page Heading -->
                                ...
                                <div class="card-body">
                                    <center><h2>Data Angsuran</h2></center><br>
                                <a href="http://127.0.0.1:8000/dashboard" class="btn btn-md btn-danger mb-3">KEMBALI KE MENU</a>
                                <a href="{{ route('angsuran.create') }}" class="btn btn-md btn-success mb-3">TAMBAH DATA</a>
                                <table class="table table-bordered">
                                <thead>
                                <tr>
                                <th scope="col">Nama Angsuran</th>
                                ...
                                <tr>
                                <td>{{ $angsuran->nm_angsuran }}</td>
                                <td>{{ $angsuran->jenis }}</td>
                                <td class="text-center">
                                    <form id="delete-form-{{ $angsuran->id }}" action="{{ route('angsuran.destroy', $angsuran->id) }}" method="POST">
                                        <a href="{{ route('angsuran.edit', $angsuran->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-sm btn-danger" onclick="deleteangsuran({{ $angsuran->id }})">HAPUS</button>
                                        @if (session('status'))
                                            <script>
                                                Swal.fire({
