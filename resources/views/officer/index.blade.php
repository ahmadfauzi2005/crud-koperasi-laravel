        ...
        <div class="container">
          ...
          <div class="navbar-collapse collapse justify-content-center" id="navbarDefault">
            <ul class="navbar-nav">
              ...
              <li class="nav-item">
                <a class="nav-link active" href="officer">Petugas</a>
              </li>
...
@endif
                            <!-- Page Heading -->
                                ...
                                <div class="card-body">
                                    <center><h2>Data Petugas</h2></center><br>
                                <a href="http://127.0.0.1:8000/dashboard" class="btn btn-md btn-danger mb-3">KEMBALI KE MENU</a>
                                <a href="{{ route('officer.create') }}" class="btn btn-md btn-success mb-3">TAMBAH DATA</a>
                                ...
                                <tr>
                                <td>{{ $officer->nama }}</td>
                                <td>{{ $officer->pasword }}</td>
                               <td>{{ $officer->nama_lkp }}</td>
                                <td>{{ $officer->contact }}</td>
                                <td>{{ $officer->pejabat }}</td>
                                <td class="text-center">
                                    <form id="delete-form-{{ $officer->id }}" action="{{ route('officer.destroy', $officer->id) }}" method="POST">
                                        <a href="{{ route('officer.edit', $officer->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-sm btn-danger" onclick="deleteofficer({{ $officer->id }})">HAPUS</button>
                                        @if (session('status'))
                                            <script>
                                                Swal.fire({
